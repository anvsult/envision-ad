...
export function useMediaList() {
    ...
    const editMedia = async (id: string | number, formState: MediaFormState) => {
        ...
        try {
            ...
            setMedia((prev) =>
                prev.map((r) => (String(r.id) === String(id) ? { ...r, name: updated.title, image: updated.imageUrl ?? r.image, status: updated.status ?? "Pending Admin Approval", price: updated.price ? `$${updated.price}` : r.price } : r))
            );
            return updated;
        } catch (err: unknown) {
